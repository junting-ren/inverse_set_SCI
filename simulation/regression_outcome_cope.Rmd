---
title: "Simulation_COPE_linear_regression_outcome"
author: "Junting Ren"
date: "12/5/2021"
...
# linear regression simulation for single level confidence set
...
## For 2d grid that is continuous
```{r}
num_levels = 5000
n_sim = 5000
...
pred_data =as.matrix(cbind(x0 = rep(1,dim(grid_df)[1]), expand.grid(x1 = x1, x2 =  x2) )) # for producing the true mean
...
levels = 3
grid_df_level = grid_df[2.9<y_true_pred_data &y_true_pred_data< 3,]
...
for(n in NVec){
  p_result = foreach(t=1:n_sim,.errorhandling = "pass", 
    ...
    SCB =  cbind(SCB_linear_outcome(df_fit = data_fit, model = model, grid_df = grid_df, n_boot = n_boot,
                                    alpha = alpha, grid_df_boot = grid_df_level),y_true = y_true_pred_data)
...
x1 = rnorm(10000)
x2 = rnorm(10000)
grid_df = data.frame( x1 = x1, x2 =  x2 )# dataset for model fit inside the function
pred_data =as.matrix(cbind(x0 = rep(1,dim(grid_df)[1]), x1 = x1, x2 =  x2 )) # for producing the true mean
# generate the true mean for checking the containment
y_true_pred_data = pred_data[,1]*beta[1] + pred_data[,2]*beta[2]+(pred_data[,2]^2)*beta[3] +(pred_data[,2]^3)*beta[4] +
  (pred_data[,3])*beta[5] +  (pred_data[,3]^2)*beta[6] +  (pred_data[,3]^3)*beta[7]
levels = 3
...
y =  X[,1]*beta[1] + X[,2]*beta[2]+(X[,2]^2)*beta[3] +(X[,2]^3)*beta[4] +
  (X[,3])*beta[5] +  (X[,3]^2)*beta[6] +  (X[,3]^3)*beta[7]+ rnorm(n, sd=sd_error)
data_fit = data.frame(y, dplyr::select(data.frame(X), x1, x2))
SCB = cbind(SCB_linear_outcome(df_fit = data_fit, model = model, 
                               grid_df = grid_df, n_boot = n_boot, 
                               alpha = alpha, grid_df_boot = grid_df_level),y_true = y_true_pred_data)
confidence_set = scb_to_cs(SCB$UpperBound,SCB$LowerBound, levels = 3,x1 = x1, x2 = x2,
                              true_mean = SCB$y_true, return_contain_only = T, return_plot = F)
confidence_set$contain_individual
...
for(n in NVec){
  p_result = foreach(t=1:n_sim,.errorhandling = "pass", 
    ...
    confidence_set = scb_to_cs(SCB$UpperBound,SCB$LowerBound, levels = levels,
                               true_mean = SCB$y_true, return_contain_only = T)
    upper5000 = confidence_set$contain_all
    cover_scb  = SCB %>% 
      mutate(contain = y_true > LowerBound & y_true < UpperBound) %>% 
      pull(contain) %>% 
...
confidence_set = scb_to_cs(scb_up_matrix,scb_low_matrix, levels = c(0.1, 0.2, 0.4,0.5, 0.6),x1 = x1, x2 = x2,
                           true_mean = true_mean_matrix, return_contain_only = T, return_plot = T)
confidence_set$plot_cs
```
...
result_df_logistic
save.image("regression_outcome.RData")
```
